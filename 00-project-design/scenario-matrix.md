...
| 04 | DNS Tunneling | `T1071.004` — Application Layer Protocol: DNS; `T1572` not claimed | first-label structure/length, fresh-child diversity, parent concentration, short-window behavior, query type and client identity | independently investigate the frozen detection and prove a proportionate DNS response | **Complete: resolver/BIND path + Detection Engineering + Dashboard/Alert + `dns_tunneling_v1` + official operator/SOC/IR exercise + human-approved RPZ redirect to `10.50.30.30` + safe reset + final closeout** |
