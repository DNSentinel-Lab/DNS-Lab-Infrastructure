...
| 04 | DNS Tunneling | `T1071.004` — Application Layer Protocol: DNS (frozen DE mapping); `T1572` not claimed by current engineering validation | first-label structure/length, fresh-child diversity, parent concentration, short-window behavior, query type and client identity | independently investigate the frozen detection and prove the approved response/no-response outcome | **Infrastructure + Detection Engineering complete: resolver/BIND path, Dashboard Studio, Detection v1.0, scheduled alert and `dns_tunneling_v1` AI evidence mapping are SOC-ready; official simulation/SOC/IR exercise next** |
