...
| 04 | DNS Tunneling | `T1071.004` — Application Layer Protocol: DNS (primary); `T1572` only if later implementation genuinely fits | label structure/length, frequency, query type, unique subdomains and client behavior | investigate encoded DNS behavior and prove block/sinkhole result | **Infrastructure ready: Scenario 02 defender path + `dns-tunnel-auth01` authoritative BIND endpoint + nested Route 53 delegation validated; Detection Engineering next** |
