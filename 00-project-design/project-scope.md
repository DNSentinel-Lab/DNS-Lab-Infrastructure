...
- **Scenario 04:** reuses the same defender DNS path and includes the implemented `dns-tunnel-auth01` authority and nested Route 53 delegation. Detection Engineering is now complete in the dedicated Scenario 04 repository: baseline/hunting, Dashboard Studio, frozen Detection v1.0, scheduled alert and `dns_tunneling_v1` AI evidence mapping are SOC-ready. The official simulation/SOC/IR execution remains pending.
